...
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (process.env.NODE_ENV !== 'production') {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!oldTree) {
<span class="cstat-no" title="statement not covered" >      throw new Error('Missing old tree during synchronization');</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!newTree) {
...
&nbsp;
  ...
  // Synchronize the top level elements.
    ...
    transaction.patches = syncTree(
      oldTree || <span class="branch-1 cbranch-no" title="branch not covered" >null,</span>
      newTree || <span class="branch-1 cbranch-no" title="branch not covered" >null,</span>
      [],
      state,
    );
  }
