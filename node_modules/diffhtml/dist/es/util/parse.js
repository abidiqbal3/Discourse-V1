...
import { VTree, Supplemental, TransactionConfig, ParserConfig, EMPTY, NODE_TYPE } from "./types.js"; // Magic token used for interpolation.
...
const selfClosingElementsDefaults = ['meta', 'img', 'link', 'input', 'area', 'br', 'hr', 'area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'];
/** @type {any} */
...
const HTMLElement = (nodeName, rawAttrs, supplemental, options) => {
  let match = null;
  const attrEx = /([_@$#a-z][^\s\x00-\x1F"'>\/=\uFDD0-\uFDEF\uFFFE\uFFFF]*)\s*(=\s*("([^"]+)"|'([^']+)'|(\S+)))?/ig; // Support dynamic tag names like: `<${MyComponent} />`.
...
export default function parse(html, supplemental, options = {}) {
  ...
  /** @type {string[]} */
  getConfig('rawElements', rawElementsDefaults, 'array', options.parser));
  const selfClosing = new Set(
  /** @type {string[]} */
  getConfig('selfClosingElements', selfClosingElementsDefaults, 'array', options.parser));
