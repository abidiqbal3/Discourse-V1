(function (global, factory) {
  ...
  function getConfig(name, defaultValue) {
    ...
    if (useEnv && keyName in process$1.env) {
      return formatValue(process$1.env[keyName], type);
    }
  ...
  var selfClosingElementsDefaults = ['meta', 'img', 'link', 'input', 'area', 'br', 'hr', 'area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'];
  ...
  var interpolateChildNodes = function (currentParent, markup, supplemental) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (i % 2 === 1) {
        ...
        var isFragment = innerTree.nodeType === NODE_TYPE.FRAGMENT;
  ...
  var HTMLElement = function (nodeName, rawAttrs, supplemental, options) {
    var match = null;
    var attrEx = /([_@$#a-z][^\s\x00-\x1F"'>\/=\uFDD0-\uFDEF\uFFFE\uFFFF]*)\s*(=\s*("([^"]+)"|'([^']+)'|(\S+)))?/ig;
    ...
    for (var _match; _match = attrEx.exec(rawAttrs || EMPTY.STR);) {
      ...
      if (name === '') {
        var _match2 = rawAttrs.match(tokenEx);

        tokenValue = _match2 ? _match2[0] : '';
      }

      var testValue = _match[6] || _match[5] || _match[4];
      var value = tokenValue || testValue || (isHTML ? _match[1] : testValue || true);
  ...
  function parse$1(html, supplemental) {
    ...
    var blockText = new Set(getConfig('rawElements', rawElementsDefaults, 'array', options.parser));
    var selfClosing = new Set(getConfig('selfClosingElements', selfClosingElementsDefaults, 'array', options.parser));
  ...
  function parseNewTree(transaction) {
    ...
    if (typeof input === 'string') {
      ...
      var _parse = parse$1(input, undefined, options),
  ...
  function toString(input) {
    ...
    var oldTree = createTree();
    var activeTasks = new Set(config.tasks || defaultTasks);
    ...
    config.tasks = _toConsumableArray(activeTasks);
    config.inner = true;
    var markup = '';

    try {
      markup = Transaction.create(oldTree, input, config).start();
    } catch (e) {
      release(oldTree);
      throw e;
    }
  ...
  function serializeAttributes(attributes) {
    var attrs = keys$1(attributes);
    return attrs.length ? ' ' + attrs.map(function (keyName) {
      var value = attributes[keyName];
      var isFalsy = !value;
      var isDynamic = typeof value === 'object' || typeof value === 'function';
      return "".concat(keyName).concat(!isFalsy && !isDynamic ? "=\"".concat(String(value), "\"") : '');
    }).join(' ') : '';
  }
  ...
  function serializeVTree(vTree) {
    ...
    if (nodeType === 11) {
      for (var i = 0; i < childNodes.length; i++) {
        ...
        output += "<".concat(tag).concat(serializeAttributes(attributes), "></").concat(tag, ">");
      } else if (nodeType === 3) {
          output += nodeValue;
        } else if (childNodes.length) {
            var children = childNodes.map(function (childNode) {
              return "".concat(serializeVTree(childNode));
            }).join('');
            output += "<".concat(tag).concat(serializeAttributes(attributes), ">").concat(children, "</").concat(tag, ">");
          }
  ...
  function handleTaggedTemplate(strings) {
    ...
    if (strings.length === 1 && !values.length) {
      ...
      var _strict = getConfig('strict', false, 'boolean', {
        strict: handleTaggedTemplate[$$strict]
      });

      delete handleTaggedTemplate[$$strict];

      var _parse = parse$1(strings[0], undefined, {
        parser: {
          strict: _strict
        }
      }),
    ...
    var strict = getConfig('strict', false, 'boolean', {
      strict: handleTaggedTemplate[$$strict]
    });

    var _parse2 = parse$1(HTML, supplemental, {
      parser: {
        strict: strict
      }
    }),
  ...
  if ($$diffHTML in globalThis) {
    ...
    if (__VERSION__ !== existingApi.VERSION) {
      console.log("Loaded ".concat(__VERSION__, " after ").concat(existingApi.VERSION));
    }
  }
