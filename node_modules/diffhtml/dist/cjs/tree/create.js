...
function createTree(input, attributes, childNodes, ...rest) {
  ...
  if (input && isObject && 'ownerDocument' in inputAsHTMLEl) {
    ...
    if (nodeType === _types.NODE_TYPE.TEXT) {
      const vTree = createTree(textName, inputAsHTMLEl.nodeValue);

      _types.NodeCache.set(vTree, inputAsHTMLEl);
    ...
    if (inputAsHTMLEl.nodeType === _types.NODE_TYPE.ELEMENT && inputAttrs && inputAttrs.length) {
    ...
    if (inputAsHTMLEl.nodeType === _types.NODE_TYPE.ELEMENT || inputAsHTMLEl.nodeType === _types.NODE_TYPE.FRAGMENT) {
      childNodes = [];
    ...
    _types.NodeCache.forEach((node, vTree) => {
      if (node === input) {
        entry = vTree;
      }
    });
    ...
    entry.attributes = _objectSpread(_objectSpread({}, entry.attributes), attributes); // Use childNodes that are passed in, otherwise keep the existing nodes.

    entry.childNodes = childNodes;

    _types.NodeCache.set(entry, inputAsHTMLEl);
  ...
  if (isTextNode) {
    ...
    entry.nodeType = _types.NODE_TYPE.TEXT;
    entry.nodeValue = String(nodeValue);
    return entry;
  } else if (entry.nodeName === fragmentName) {
    entry.nodeType = _types.NODE_TYPE.FRAGMENT;
  } else if (input === '#comment') {
    entry.nodeType = _types.NODE_TYPE.COMMENT;
  } else {
    entry.nodeType = _types.NODE_TYPE.ELEMENT;
  ...
  if (useNodes && allNodes.length && (!attributes || !attributes.childNodes)) {
    for (let i = 0; i < allNodes.length; i++) {
      ...
      if (isArray(newNode)) {
        ...
        else if (newNode.nodeType === _types.NODE_TYPE.FRAGMENT && typeof newNode.rawNodeName === 'string') {
  ...
  if (attributes && typeof attributes === 'object' && !isArray(attributes)) {
    entry.attributes = _objectSpread({}, attributes); // Use childNodes directly from the attributes.
  ...
  if (_types.CreateTreeHookCache.size) {
    _types.CreateTreeHookCache.forEach((fn, retVal) => {
