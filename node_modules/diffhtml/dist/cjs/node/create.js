...
function createNode(vTreeLike, ownerDocument = _global.default.document, isSVG) {
  ...
  const existingNode = _types.NodeCache.get(vTree); // If the DOM Node was already created, reuse the existing node. This is
  ...
  _types.CreateNodeHookCache.forEach(fn => {
  ...
  _types.NodeCache.set(vTree, validNode); // Append all the children into the domNode, making sure to run them
