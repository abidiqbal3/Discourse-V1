...
class Transaction {
  ...
  static invokeMiddleware(transaction) {
    ...
    _types.MiddlewareCache.forEach(fn => {
      const label = `invoke ${fn.name || 'anon'}`;
      measure(label); // Invoke all the middleware passing along this transaction as the only
  ...
  constructor(mount, input, config) {
    _defineProperty(this, "state", _types.EMPTY.OBJ);

    _defineProperty(this, "mount", _types.EMPTY.OBJ);

    _defineProperty(this, "input", _types.EMPTY.OBJ);
    ...
    const hasObserver = ('MutationObserver' in (_global.default.window || _types.EMPTY.OBJ));
    this.state = _types.StateCache.get(mount) ||
    /** @type {TransactionState} */
    {
      measure: (0, _makeMeasure.default)(this),
      svgElements: new Set(),
      scriptsToExecute: new Map(),
      activeTransaction: this,
      mutationObserver: hasObserver && new _global.default.window.MutationObserver(isDirtyCheck)
    };
    this.tasks =
    /** @type {Function[]} */
    (0, _config.default)('tasks', defaultTasks, undefined, config).slice(); // Store calls to trigger after the transaction has ended.

    this.endedCallbacks = new Set();

    _types.StateCache.set(mount, this.state);
  }
  /**
   * @return {Promise<Transaction> | unknown}
   */
  ...
  start() {
    ...
    const takeLastTask = tasks.pop(); // Start measuring a render for performance tracing.
   ...
   */
    ...
    state.isDirty = false; // If MutationObserver is available, look for changes.
    ...
    scriptsToExecute.forEach((type = _types.EMPTY.STR, vTree) => {
      const oldNode =
      /** @type {HTMLElement} */
      _types.NodeCache.get(vTree); // Reset the type value.


      if (type) oldNode.setAttribute('type', type);else oldNode.removeAttribute('type');
    }); // Save the markup immediately after patching.

    state.previousMarkup = 'outerHTML' in mountAsHTMLEl ? mountAsHTMLEl.outerHTML : _types.EMPTY.STR; // Only execute scripts if the configuration is set. By default this is set
    ...
    if (config.executeScripts) {
      ...
      scriptsToExecute.forEach((_, vTree) => {
        const oldNode = _types.NodeCache.get(vTree);
        ...
        if (_types.StateCache.has(oldNode)) {
          (0, _release.default)(oldNode);

          _types.StateCache.set(newNode, state);
    ...
    scriptsToExecute.clear(); // Defer garbage collection if the current size is more than double the
    // current allocation.

    if (_pool.default.memory.free.size < _pool.default.memory.allocated.size * 2) {
      (0, _memory.gc)();
    ...
    this.endedCallbacks.clear(); // Mark the end to rendering.
