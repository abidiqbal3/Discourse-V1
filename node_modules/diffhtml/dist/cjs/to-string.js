...
function toString(input, config = {}) {
  const oldTree = (0, _create.default)();
  const activeTasks = new Set(config.tasks || _transaction.defaultTasks); // Replace the `endAsPromise` task with the string return value.

  activeTasks.delete(_transaction.tasks.endAsPromise);
  activeTasks.add(function endAsString(
  /** @type {Transaction} */
  transaction) {
  ...
  try {
    markup =
    /** @type {string} */
    _transaction.default.create(oldTree, input, config).start();
  } catch (e) {
    (0, _release.default)(oldTree);
    throw e;
  }
